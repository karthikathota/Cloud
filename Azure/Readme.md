...
Often called as **_autoscaling_** in cloud computing.  
...
In computing idenitity refers to a person, applicaiton or a device. Examples of identiy, Karthik, email, Time Entry Apllication, etc.
In general to prove identity we can use a password, certificate or a special type of key.

## Microsoft Entra Id

Entra is part of Microsoft's broader Azure identity and security offerings. Instead of creating an own seciruty backend, users can use the API's to Entra Id to help in authentication and authorization.

## Resource Locks

There are 2 types of resoruce locks in azure

1. Read Only Locks:- Only allows u to see the reosurce and no changes to the resource can be made.
2. Cannot Delete Locks:- Only blocks the delete operation

We can use Role Based Acess Control(RBAC) to apply these locks efficiently

## Azure Policy

Azure Policy to implement standards for your organization across Azure. Some useful governance actions you can enforce with Azure Policy include: ensuring your team deploys Azure resources only to allowed regions,
enforcing the consistent application of taxonomic tags, and requiring resources to send diagnostic logs to a Log Analytics workspace.

## Microsoft Purview

It is a centralized dashboard for all the data governance issues

### Information Protection

Know Your Data --> Protect Your Data --> Prevent Loss --> Govern Your Data

Know your data:- Where the sensitive data is stored at.  
Protect The data:- Label the data as sensitive or other classes, encryption,
Prevent Loss:- Detect risky behaviours like sharing etc.  
Govern Your Data:- Automatically store, retain, delte data in compliant manner.

### Insider Risk Management

Identity potential malicious employees, IP thefts, Data Leakages, Security Violations.

## Azure ARC

It allows users to manage VM and Containers as if they were VM servers and containers. So, it is a cross platform hybrid connector.

Allows management of servers across environments.  
Supports data services outside of azure. Also clusters that are outside azure.

In the contect of servers with the help of ARC we can Manage windows and linux servers that are running outside of azure.
